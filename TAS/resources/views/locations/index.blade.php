    ...
    <div class="d-flex align-items-center">
        ...
        <div>
            <div class="col-xl-4 order-1 order-xl-2 m--align-right">
                   @if($add_pv==1)
                <a href="{{route('location_create')}}" class="btn btn-primary m-btn m-btn--custom m-btn--icon m-btn--air m-btn--pill">
                    <span>
    ...
    <div class="m-portlet m-portlet--mobile">
        ...
        <div class="m-portlet__body">
            ...
            <table class="table table-bordered table-hover" id="" width: "100%">
                ...
                <tbody id='tb_sr'>
                    @foreach($location_view as $locations)
                    <tr style="text-align: center;">
                        <td>{{$locations->id}}</td>
                        <td>{{$locations->location_code}}</td>
                        <td>{{$locations->location_name}}</td>
                        <td>{{$locations->country_name}}</td>
                        <td>{{$locations->dist_name}}</td>
                        
                        <td>{{$locations->short_des}}</td>
                        <td class="m-datatable__cell--right">
                               @if($edit_pv==1)
                            <a href="{{route('location_edit',$locations->id)}}" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit details">
                                <i class="la la-edit"></i>
                            </a>
                               @endif

                               @if($del_pv==1)
                            <a id="{{$locations->id}}" onclick="deleteAccept({{$locations->id}}) "
