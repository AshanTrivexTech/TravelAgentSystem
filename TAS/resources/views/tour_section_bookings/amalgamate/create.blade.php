    ...
    <div class="d-flex align-items-center">
        <div class="mr-auto">
            <h3 class="m-subheader__title m-subheader__title--separator ">
                Create an Amalgamate Quotation
				</h3>
                ...
                <div class="m-portlet__head-title">
                    ...
                    {{-- <div style="padding-top:20px;" class="row">
                                <div style="padding:0px;" class="col-sm-4"> --}}
                                                ...
                                                Amalgamate Quotation /  MAIN TOUR - {{$tour_quote_data->tour_code}} / {{$tour_quote_data->title}}  -  {{$tour_quote_data->frm_date}} - {{$tour_quote_data->to_date}}
                                                </h3>
                                            {{-- </div>
                                            <div style="padding:0px;" class="col-sm-4">
                                                <h3 class="m-portlet__head-text m--font-info">
 ...
 <form id="add_holetFrm" class="" method="POST" action="">
   ...
   <div class="m-portlet__body">
        <div class="form-group m-form__group row">
            ...
            value="{{$tour_quote_data->title}}"  placeholder="Enter Tour Title">
            ...
            <div class="col-lg-2">
                ...
                <select class="form-control"  id="exc_type">
                    <option value="1" selected>Pre</option>
               ...
               <div class="m-form__group form-group">
					...
					<div class="m-radio-list">
                            <label class="m-radio m-radio--state-success">
                                <input type="radio" checked class="with-gap" name="date_range" value="days" id="date_range1">
                                    Days
                                <span></span>
                            </label>
        ...
        <div class="form-group m-form__group row">
		 ...
		 <div class="form-group m-form__group row">
            ...
            <div class="col-lg-6">
                ...
                <select disabled class="form-control m-bootstrap-select m_selectpicker" data-live-search="true"  name="base_currencies"  id="base_currencies">
                    <option value="0">Please Select</option>
                    @foreach ($currency_list as $item)
                    
                         <option @if($tour_quote_data->currency_id == $item->id) selected @endif value="{{$item->id}}">{{$item->code.' - '.$item->type}}</option>
         ...
         <div class="form-group m-form__group row">
            <div class="col-lg-4">
                ...
                <select disabled class="form-control m-bootstrap-select m_selectpicker" data-live-search="true" name="market_id"  id="market_id">
                      
                    <option value="0">Please Select</option>
                    @foreach ($maket_list  as $item)
                    
                     <option @if($tour_quote_data->market_id == $item->id) selected @endif value="{{$item->id}}">{{$item->m_code.' - '.$item->market_name}}</option>
            ...
            <div class="col-lg-4">
                ...
                <select disabled class="form-control"  id="agent_id">
                        <option value="0">Please Select</option>
                            @foreach($agents as $agent)

                                 <option @if($tour_quote_data->agent_id == $agent->id) selected @endif value="{{$agent->id}}">{{$agent->ag_name}}</option>
              ...
              <div class="col-lg-4">
                ...
                <select disabled class="form-control m-bootstrap-select m_selectpicker" data-live-search="true" name="branch_id" id="branch_id">
                    <option value="0">Please Select</option>
                    @foreach ($branch_list  as $item)
                    
                     <option @if($tour_quote_data->branch_id == $item->id) selected @endif value="{{$item->id}}">{{$item->branch_code.' - '.$item->branch_name}}</option>
        ...
        <div class="form-group m-form__group row">
            ...
            <input type="number"  class="form-control m-input num-only" min="1" max="{{$tour_quote_data->pax_adult}}" value="0"  id="no_of_packs_adult" name="pax">
                <span class="m-form__help">
            ...
            <div class="col-lg-4">
                ...
                <input id="no_of_packs_children" name="pax_children" min="0" max="{{$tour_quote_data->pax_child}}" type="number" class="form-control m-input num-only" value="">
...
<script>
    ...
    $(document).ready( function(){
        ...
        $( "#frm_submit").click(function( event ) {
                ...
                var max_ad_pax = parseInt('{{$tour_quote_data->pax_adult}}');
                var max_chd_pax = parseInt('{{$tour_quote_data->pax_child}}');
                ...
                var tour_id = parseInt('{{$tour_quote_data->tour_id}}');

                var qtType = $('#exc_type').val();
                
    

                if(pax_adult>max_ad_pax){
                    $('.alert').show();
                    $('#notify').html('Sorry! Maximum Adult Pax must be '+max_ad_pax);
                }else if(pax_child>max_chd_pax){
                    $('.alert').show();
                    $('#notify').html('Sorry! Maximum Child Pax must be '+max_chd_pax);
                  ...
                  $.ajax({
                    ...
                    url: '{{Route('tour_booking_amalgamate_store')}}',
                    method: "POST",
                    data: {title:title,days:days,frm_date:frm_date,to_date:to_date,com_rate:com_rate,
                             ...
                             qtType:qtType,version_no:version_no,tour_id:tour_id},
