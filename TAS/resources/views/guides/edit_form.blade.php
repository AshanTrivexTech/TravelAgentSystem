...
<div class="m-content">
    ...
    <form id="add_holetFrm" method="POST" class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed" >
            {{csrf_field()}}
    <input type="hidden" id="sup_id" name="sup_id" value="{{$supplier->id}}"/>
            ...
            <div class="m-portlet__body">
                <div class="form-group m-form__group row">
                    <div class="col-lg-2">
                        ...
                        <input required placeholder="unique code" id="code" name="code" type="text" class="form-control"  value="{{$supplier->code}}">
                    ...
                    <div class="col-lg-6">
                        ...
                        <input required placeholder=" name" id="name" name="name" type="text"  class="form-control"  value="{{$supplier->sup_name}}" >
                        <span class="form-group text-danger"> * Required</span>
                    </div>
                    <div class="col-lg-4">
                            <label class="form-control-label">
                            ...
                            <select required name="guide_type" id="guide_type" class="form-control">
                                @foreach($guides->all() as $guide_type)
                                    <option value="{{$guide_type->id}}" @if($guide_type->id==$guide_type->guide_type_id) selected @endif>{{$guide_type->g_type}}</option>
                                @endforeach  
                            </select>
                            <span class="form-group text-danger"> * Required</span>
                        </div>
                ...
                <div class="form-group m-form__group  row">
                    <div class="col-lg-4">
                        ...
                        <textarea placeholder="Address" id="address" name="address" class="form-control" required>{{$supplier->address}}</textarea>     
                ...
                <div class="form-group m-form__group  row">
                        <div class="col-lg-3">
                                ...
                                @foreach($country_list->all() as $country)
                                    <option value="{{$country->id}}" @if($country->id==$supplier->country_id) selected @endif>{{$country->country_name}}</option>
                                @endforeach
                                </select>
                        ...
                        <div class="col-lg-3">
                            ...
                            <select required name="district" id="district" class="form-control">  
                                @foreach($district_list->all() as $district)
                                    <option value="{{$district->id}}" @if($district->id==$supplier->district_id) selected @endif>{{$district->dist_name}}</option>
                                @endforeach 
                            </select>
                        ...
                        <div class="col-lg-3">
                                ...
                                <select required name="city" id="city" class="form-control">
                                   @foreach($city_list->all() as $city)
                                        <option value="{{$city->id}}" @if($city->id==$supplier->city_id) selected @endif>{{$city->city_name}}</option>
                                    @endforeach   
                                </select>
                ...
                <div class="form-group m-form__group  row">
                        <div class="col-lg-4">
                              ...
                              <input placeholder="Email" id="email" name="email" type="email" class="form-control" value="{{$email_A->contact}}" required> 
                              <span class="form-group text-danger"> * Required</span>
                              <br>                       
                        </div>
                </div>
                <div class="form-group m-form__group  row">
                    ...
                    <div class="col-lg-6">
                          ...
                          <input placeholder="011XXXXXXXX" id="mobile" name="mobile" type="tel" class="form-control mxl_phone"  value="{{$mobile->contact}}" required >
                    </div>
                ...
                var status;
                 ...
                 else
                 {
                  ...
                  $.ajax({
                    ...
                    url: '{{Route('update_guide')}}',
                    method: "POST",
                    data: {supID:supID,code:code,name:name,guide_type:guide_type,address:address,city:city,district:district,country:country,email:email,
                           tel:tel,mobile:mobile,status:status},
                    success: function(data) {
                       ...
                       console.log(data);
                        ...
                        if(data=='updated'){
                            $('.alert').hide();
                            swal("Hotel Add successfully completed!", ""+name, "success");
