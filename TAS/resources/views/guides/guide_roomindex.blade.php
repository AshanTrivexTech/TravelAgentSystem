    ...
    <div class="d-flex align-items-center">
        ...
        <div>
            <div class="col-xl-4 order-1 order-xl-2 m--align-right">
                   ...
                   @if($add_pv==1)
                <a href="{{route('guideroom_create')}}" class="btn btn-primary m-btn m-btn--custom m-btn--icon m-btn--air m-btn--pill">
                    <span>
        ...
        <div class="m-portlet__body">
            ...
            <table class="table table-bordered table-hover table-checkable">
                ...
                <tbody id="tb_sr">
                    ...
                    @foreach($guide_rooms->all() as $guide_room)
		           <tr> 
                          <td style="text-align: center">{{$guide_room->id}}</td>
                          <td>{{$guide_room->sup_name}}</td>
                          <td>{{$guide_room->gr_types}}</td>
                          <td>{{$guide_room->type}}</td>
                          <td>{{number_format($guide_room->amount,2)}}</td>
                        <td class="m-datatable__cell--right" style="text-align: center">
                            
                               @if($edit_pv==1)
                            <a href="{{route('guideroom_edit',$guide_room->id)}}" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit details">
                                <i class="la la-edit"></i>
                            </a>
                              @endif

                              @if($del_pv)
                            <a id= "{{$guide_room->id}}" onclick="deleteAccept({{$guide_room->id}})"
