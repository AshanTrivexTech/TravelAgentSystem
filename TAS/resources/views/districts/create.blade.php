...
@section('content')
    ...
    <div class="m-portlet">
        ...
        <form class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed" method="POST" id="add_districtFrm">
                  {{csrf_field()}}
                    <div class="m-portlet__body">
                        	...
                        	<input placeholder="unique name" id="name" name="name" type="text" class="form-control m-input mxl_cityname"   value="" >
											...
											<div class="form-group m-form__group has-danger row">
													...
													@foreach($country_name->all() as $country)
			...
			$( "#frm_submit").click(function( event ) {
					...
					// console.log(code);
					  ...
					  $.ajax({
						...
						method: "POST",
						data: {code:code,name:name,country:country},
						success: function(data) {
							...
							if(data=='added'){
								$('.alert').hide();
								swal("District Add successfully completed!", ""+name, "success");
								window.location.replace('{{route('district_index')}}');
								$("#add_districtFrm")[0].reset();
								$("html, body").animate({
										scrollTop: 0
									});                               
							}else{
